document.addEventListener("DOMContentLoaded", function () {
  ...
  function sortByScore() {
    ...
    resultItems.sort((a, b) => {
      const scoreA = parseFloat(
        a.querySelector(".key").textContent.match(/Skor: ([\d.]+)/)[1]
      );
      const scoreB = parseFloat(
        b.querySelector(".key").textContent.match(/Skor: ([\d.]+)/)[1]
      );
  ...
  function decryptText() {
    ...
    if (!ciphertext) {
      //   alert('Silakan masukkan teks tersandi.');
      Swal.fire({
        icon: "error",
        title: "Oops...",
    ...
    if (!keyLength || keyLength <= 0 || keyLength > 5) {
      //   alert('Silakan masukkan panjang kunci antara 1-5.');
      Swal.fire({
        icon: "error",
        title: "Oops...",
  ...
  keyLengthInput.addEventListener("input", function () {
    if (this.value > 5) {
      this.value = 5;
      //   alert('Maksimum panjang kunci adalah 5 untuk mencegah waktu pemrosesan yang terlalu lama.');
      Swal.fire({
        icon: "error",
        title: "Oops...",
